...
class EditCashIn extends StatefulWidget {
  final String cashInId;
  const EditCashIn({super.key, required this.cashInId});
  @override
  State<EditCashIn> createState() => _EditCashInState();
}

class _EditCashInState extends State<EditCashIn> {
  List accountLists = [];
  ...
  void initState() {
    super.initState();
    fetchCashInDetails(widget.cashInId);
    loadAccounts();
  }
  ...
  Future<void>fetchCashInDetails(String id) async {
    setState(() => isLoading = true);
    Map res = await getCashInByIdReq(id);
    if (res["success"] == true) {
      Map data = res["data"];
      setState(() {
  ...
  void handleFormSubmit()async{
    ...
    if (date == null || date.trim().isEmpty) {
      ...
      if(res["success"] == true){
        showSuccessToast(context,"CashIn add success");
